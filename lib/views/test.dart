    ...
    return SizedBox(
      ...
      child: Row(
        ...
        children: [
          IconButton(
            onPressed: () {
              final timeData = ref.read(timeDataPro);
              ref.read(timeDataPro.notifier).handelSessionChange(
                    sessionStartDt: timeData.sessionData.dayStartTime,
                    sessionEndDt: timeData.sessionData.dayEndTime,
                  );
            },
          ...
          IconButton(
            onPressed: () {
              final oldStartTime =
                  ref.read(timeDataPro).sessionData.sessionStartDt;
              ...
              ref.read(timeDataPro.notifier).handelSessionChange(
                    sessionStartDt: newStartTime,
                    sessionEndDt:
                        ref.read(timeDataPro).sessionData.sessionEndDt,
                  );
            },
          ...
          IconButton(
            onPressed: () {
              final sessionData = ref.read(timeDataPro).sessionData;
              final oldSesEnd = sessionData.sessionEndDt;
              final newSesEnd = oldSesEnd.subtract(
                const Duration(minutes: 30),
              );
              final tic = ref.read(ticPro);
              if (!newSesEnd.isAfter(tic)) return;

              ref.read(timeDataPro.notifier).handelSessionChange(
                    sessionStartDt:
                        ref.read(timeDataPro).sessionData.sessionStartDt,
                    sessionEndDt: newSesEnd,
                  );
            },
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Stack(
      children: [
        GraphWrapper(
          GridLabelPainter(
            sessionData: appDataProvider.sessionData,
            statData: appDataProvider.statData,
          ),
          height: height,
          width: width,
        ),
        GraphWrapper(
          PreviousDaysPathPainter(
            previousDaysCoordinates:
                appDataProvider.coordinates.session.unactiveObjects,
          ),
          height: height,
          width: width,
        ),
...
final todayPathCoo = Provider<List<Offset>>((ref) {
  ...
  return timeData.coordinates.session.today;
});
...
class PreviousDaysPathPainter extends CustomPainter {
  ...
  @override
  void paint(Canvas canvas, Size size) {
    // debugPrint('## PreviousDaysPathPainter');
    // debugPrint('## Days: ${previousDaysCoordinates.length}');
    Paint paint = Paint()
      ..color = Colors.black
      ..strokeWidth = 1.0
      ..style = PaintingStyle.stroke;
